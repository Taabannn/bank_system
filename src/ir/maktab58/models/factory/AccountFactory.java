...
public class AccountFactory {
    public static Account getAccount(String type, long initialBalance, long accountNumber, Owner owner) {
        Account account;
        if (AccountType.valueOf(type).equals(AccountType.SHORT_TERM_SAVING)) {
            account = ShortTermAccount.builder()
                    .withAccountNumber(accountNumber)
                    .withBalance(initialBalance)
                    .withOwner(owner)
                    .withCreationDate(new Date()).build();
        } else if (AccountType.valueOf(type).equals(AccountType.LONG_TERM_SAVING)) {
            account = LongTermSavingAccount.builder()
                    .withAccountNumber(accountNumber)
                    .withBalance(initialBalance)
                    .withOwner(owner)
                    .withCreationDate(new Date()).build();
        } else if (AccountType.valueOf(type).equals(AccountType.TRANSACTION)) {
